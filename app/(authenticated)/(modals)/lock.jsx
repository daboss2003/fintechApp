...
const Page = () => {
  ...
  useEffect(() => {
    LocalAuthentication.isEnrolledAsync().then(data => setIsEnrolled(data)).catch(err => console.log(err));
    AsyncStorage.getItem('userCode').then(data => data !== null && setUserCode(data)).catch(err => console.log(err))
  }, []);
  ...
  return (
    <SafeAreaView>
      <Text style={styles.greeting}>Welcome back, {firstName || user.emailAddresses[0] || user.phoneNumbers[0]}</Text>
          ...
          <View style={{minWidth: 30}}>
              ...
              <TouchableOpacity onPress={onBiometricAuthPress}>
                <View style={{flexDirection: 'row', gap: 4, alignItems: 'center'}}>
                  <MaterialCommunityIcons name="face-recognition" size={20} color="black" />
                  <Text>/</Text>
